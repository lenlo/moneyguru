...
class TestCasePristine:
    ...
    def test_set_empty_text(self):
        # Setting up empty text sets the date to None
        self.w.text = ''
        eq_(self.w.date, None)

    def test_set_invalid_date_as_text(self):
        # Setting invalid date as text does *not* set the date to None, because we don't want to
        # mess with the date of a widget that's in the middle of an edit session.
        self.w.text = '0 /08/2009'
        eq_(self.w.date, date.today())
